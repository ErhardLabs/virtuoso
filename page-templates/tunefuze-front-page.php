...
function virtuoso_publisher_front_page_content() {
  ...
  if (is_front_page()) {
    ...
    add_filter( 'body_class', function( $classes ) {
      return array_merge( $classes, array( 'tunefuze', 'all_fuzes' ) );
    } );
...
//    $profileOwner = ((int) buddypress()->displayed_user->id === (int) buddypress()->loggedin_user->userdata->ID);
    ...
    if (is_user_logged_in()) {
      ?>
      <h2>Recommended</h2>
      <div id="recommended_fuzes"></div>
      <h2>Trending</h2>
      <div id="trending_fuzes"></div>
      <h2>Curated</h2>
      <div id="curated_fuzes"></div>
      <h2>Following</h2>
      <div id="following_fuzes"></div>
      <h2>All-Time Most Viewed</h2>
      <div id="all_time_most_viewed_fuzes"></div>
      <h2>All-Time Most Liked</h2>
      <div id="all_time_most_liked_fuzes"></div>
      <?php
    } else {
      ?>
      <h2>Trending</h2>
