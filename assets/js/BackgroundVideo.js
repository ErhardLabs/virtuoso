class BackgroundVideo {
	...
	constructor() {
		...
		document.addEventListener( 'DOMContentLoaded', function( event ) {
			...
			// Don't load scripts if there isn't a video.
			if ( 'undefined' != typeof self.homeVideo ) {
	...
	addVideoHTML() {
		...
		if ( this.homeVideo || this.playlistIDs ) {
			...
			// offset += (document.getElementById('wpadminbar')) ? document.getElementById('wpadminbar').offsetHeight :
			// 0;
			offset += 'px';

			if ( 576 < $( window ).width() ) {

				// let src = 'https://www.youtube.com/embed/' + this.homeVideo +
				// '?rel=0&controls=0&showinfo=0&autoplay=1&disablekb=1&loop=1&enablejsapi=1';
				...
				// INSERT VIDEO PLAYER
				$( 'body' ).prepend( '<div class="video-background">' + '<div class="video-foreground">' + '<iframe class="home_video ' + blur + '" id="yt_home_embed" width="2460" height="1440" src="' + src + '" frameborder="0" allowfullscreen style="top: ' + offset + '" data-autoplay="1"></iframe>' + '<div class="hero-overlay-fade" ></div>' + '<div class="hero-overlay" ></div>' + '</div>' );
